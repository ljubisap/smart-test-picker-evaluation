...
Running PIT with `targetTests=org.jgrapht.*` is intractable — the coverage scan phase exceeds 60 minutes per class due to 2308 tests × N mutations × fullMutationMatrix. Scoping to the subpackage reduces to seconds-to-minutes while maintaining validity.
