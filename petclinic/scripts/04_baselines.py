...
def load_mutations(results_dir):
    """Load all KILLED mutations from mutations.xml (single file or per-class dirs)."""
    mutations = []

    # Try single mutations.xml first (PetClinic format)
    single_xml = results_dir / "mutations.xml"
    if single_xml.exists():
